...
?>
      ...
      <main class="main">
        ...
        <td class="text-center">購物金額</td>
        <td class="text-center"></td>
      </tr>
<?php
$sql = "SELECT `member`.id, `member`.username, `member`.`level` , `member`.joindate, `member`.email,SUM(`order_details`.quantity*`order_details`.unitprice)
FROM `member`
LEFT JOIN `order`
ON `member`.id=`order`.user_id
...
while ($st = $stmt->fetch()) {
    ...
    echo '</td>
        ...
        <td class="text-center">';if ($st['SUM(`order_details`.quantity*`order_details`.unitprice)'] == null) {
        echo 0;
    } else {
        echo $st['SUM(`order_details`.quantity*`order_details`.unitprice)'];
    }
    ;
    echo ' $</td>
        <td class="text-center"></td>
